...
export function GET() {
  ...
  const lines = [
    ...
    `- Gate (before you trade/transfer): b20-authenticity (run FIRST — real B20 or lookalike?), b20-gate (per-token GO/HOLD/STOP), b20-transfer-preflight (will THIS from→to transfer clear now — the per-payment check).`,
