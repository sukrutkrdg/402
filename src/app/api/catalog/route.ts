  ...
  return Response.json(
    ...
    decisionReceipt: {
      ...
      fields: {
        ...
        refundable: "boolean — whether this call qualifies for a refund (a refusal is never billed, on either payment path) (verdict checks)",
        refundRule: "the enforced rule: a refusal (confidence=low, core feed down) is auto-refunded on the credit path, and returned as 502 on the x402 path so the payment never settles; both stamp x-refunded:true. Full-confidence verdicts are final (verdict checks)",
      },
      tiers: {
      ...
      refund: {
        rule: "refusal (core feed unavailable) is never billed",
        credits: "the debit is auto-refunded; 200 with paidVia:'credits-refunded'",
        x402: "returned as HTTP 502 so the payment is never settled; body carries paidVia:'not-settled' and the full receipt",
        header: "x-refunded: true",
      },
