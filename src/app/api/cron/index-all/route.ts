...
export async function GET(req: NextRequest) {
  ...
  let attempts = 0;
  let spent = 0;
  /** Price of the one over-cap purchase a run may make, deliberately kept out
   *  of `spent` so it does not consume the budget it was exempted from. */
  let exempted = 0;
  let stale = 0;
  let aiSkipped = 0;
  ...
  for (const { s } of staleSet) {
    ...
    // The exempted purchase does not eat the budget it was exempted from.
    //
    // `spent` counts only what the cap is meant to govern. When an over-cap
    // service is let through as the first purchase, its price is recorded in
    // `exempted` instead — otherwise it lands in `spent`, immediately exceeds
    // the cap on its own, and every remaining service is deferred.
    //
    // That is what happened once the conveyor began sorting by urgency on
    // 2026-09-05: a 75c report reached the front, and runs that reported
    // healthy were settling exactly one service a day. Eight live endpoints —
    // b20-gate, b20-guard, b20-batch, token-price and four more — aged past the
    // 30-day eviction window and fell out of discovery before anyone noticed,
    // because the cron's own output looked like a normal run.
    //
    // Worst case is unchanged and is still what the comment above promises:
    // cap plus one price.
    const exempt = spent === 0 && exempted === 0 && cents > MAX_SPEND_CENTS;
    const overCap = !exempt && spent + cents > MAX_SPEND_CENTS;
